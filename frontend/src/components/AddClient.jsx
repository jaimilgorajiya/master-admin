...
const AddClient = ({ onBack, onSuccess }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    clientEmail: "",
    clientPhone: "",
    clientType: 'service', // Fixed to service
    serviceIds: [],
    packageIds: [],
  });
  ...
  const fetchPackages = async () => {
    try {
      ...
      if (res.data.success) {
        // Filter for service packages specifically (matching PackageManagement.jsx)
  ...
  const handleServiceChange = (e) => {
    ...
    setDiscountInfo(null); // Reset discount when selection changes
  };
  ...
  const calculateTotal = () => {
      ...
      return { 
        total: discountInfo ? (originalTotal - discountInfo.discount) : originalTotal, 
        serviceTotal, 
        pkgTotal,
        originalTotal
      };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API}/api/client/create`, submitData, { headers: authHeaders() });
    ...
    <div className="drawer-overlay active" onClick={onBack}>
      <div className="side-drawer active" onClick={e => e.stopPropagation()}>
          ...
          <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
            ...
            <div className="drawer-body" style={{ textAlign: 'center', padding: '40px 24px' }}>
                ...
                <div style={{ 
                    ...
                    textAlign: 'left', marginBottom: '32px', border: '1px solid rgba(255,255,255,0.05)',
                    ...
                    <div style={{ marginBottom: '16px' }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                            {[...(doneClient.selectedPackages || []), ...(doneClient.selectedServices || [])].map((item, i) => (
                                <span key={i} style={{ 
                                    fontSize: '12px', padding: '4px 10px', background: 'rgba(255,255,255,0.05)', 
                                    borderRadius: '6px', color: 'var(--text-secondary)', border: '1px solid rgba(255,255,255,0.05)' 
                                }}>
                                    {item}
                                </span>
                            ))}
                    ...
                    <div>
                        ...
                        <span style={{ 
                            ...
                            fontWeight: 800, textTransform: 'uppercase', border: '1px solid rgba(255, 149, 0, 0.2)' 
                        }}>
                            Pending Payment
                        </span>
                    </div>
                ...
                <button className="btn-primary" style={{ width: '100%', padding: '16px', fontSize: '16px', fontWeight: 700 }} onClick={onBack}>
                    Return to Dashboard
                </button>
            </div>
            ...
            <div className="billing-step">
                <div className="premium-group relative">
                    ...
                    <div className={`custom-select-trigger ${isDropdownOpen ? 'active' : ''}`} onClick={(e) => { e.stopPropagation(); setIsDropdownOpen(!isDropdownOpen); setIsPackageDropdownOpen(false); }}>
                        <span>{formData.serviceIds.length === 0 ? "Select Services" : `${formData.serviceIds.length} Selected`}</span>
                        <span className="chevron">▼</span>
                    </div>
                    {isDropdownOpen && (
                        <div className="premium-dropdown" onClick={e => e.stopPropagation()}>
                            {serviceList.length === 0 ? (
                                <div className="dropdown-empty">No services found</div>
                            ) : (
                                serviceList.map(s => (
                                    <label key={s._id} className="dropdown-item">
                                        <input type="checkbox" value={s._id} checked={formData.serviceIds.includes(s._id)} onChange={handleServiceChange} />
                ...
                <div className="premium-group relative mt-6">
                    ...
                    {isPackageDropdownOpen && (
                        <div className="premium-dropdown" onClick={e => e.stopPropagation()}>
                            {packages.length === 0 ? (
                                <div className="dropdown-empty">No packages found</div>
                            ) : (
                                packages.map(p => (
                ...
                <div className="order-summary-box">
                    ...
                    <div className="summary-row">
                        ...
                        <span>₹{currentTotal.serviceTotal}</span>
                    </div>
                    <div className="summary-row">
                        <span>Packages Total</span>
                        <span>₹{currentTotal.pkgTotal}</span>
                    </div>
                    ...
                    <div style={{ marginTop: '20px', paddingTop: '20px', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
                        ...
                        <div style={{ display: 'flex', gap: '10px' }}>
                            ...
                            <button 
                                ...
                                style={{ 
                                    padding: '0 24px', background: discountInfo ? '#34c759' : 'var(--accent-primary)', color: 'black', 
                                    border: 'none', borderRadius: '12px', fontSize: '13px', fontWeight: 700, cursor: 'pointer',
                                    opacity: (applyingCoupon || !couponCode) ? 0.5 : 1,
                                    transition: 'all 0.3s',
                                    boxShadow: discountInfo ? '0 4px 15px rgba(52,199,89,0.3)' : '0 4px 15px rgba(0,200,255,0.3)'
                                }}
                            >
                        ...
                        {discountInfo && (
                            <div style={{ 
                                ...
                                background: 'rgba(52,199,89,0.05)', border: '1px dashed rgba(52,199,89,0.3)',
                                fontSize: '12px', color: '#34c759', display: 'flex', justifyContent: 'space-between', alignItems: 'center'
                            }}>
                    ...
                    {discountInfo && (
                        <div className="summary-row" style={{ color: '#34c759', marginTop: '12px', fontWeight: 600 }}>
                            <span>Discount</span>
                    ...
                    <div className="total-row">
                        <span>Total Due</span>
                        <span className="grand-total">₹{currentTotal.total}</span>
                    </div>
                </div>
            </div>
          )}
        ...
        <div className="drawer-actions">
          <button className="btn-secondary" onClick={() => step > 1 ? setStep(1) : onBack()}>
            {step === 1 ? "Cancel" : "Back"}
          </button>
          {step === 1 ? (
            ...
            <button className="btn-primary" onClick={handleSubmit} disabled={loading}>
              {loading ? "Processing..." : "Create Client & Send Link"}
            </button>
          )}
          </div>
          </>
      ...
      <style>{`
        ...
        .side-drawer {
          ...
          background: #0f0a28; border-left: 1px solid rgba(255,255,255,0.1);
        ...
        .step-number { 
          ...
          margin-bottom: 8px; border: 2px solid rgba(255,255,255,0.1); transition: 0.3s;
        }
        ...
        .form-group input { 
          ...
          padding: 12px 16px; border-radius: 12px; color: #fff; outline: none; transition: 0.3s;
        }
        ...
        .custom-select-trigger { 
          ...
          border-radius: 14px; display: flex; justify-content: space-between; cursor: pointer; transition: 0.3s;
        }
        .custom-select-trigger:hover, .custom-select-trigger.active { border-color: var(--accent-primary); }
        
        .premium-dropdown { 
          position: absolute; top: 100%; left: 0; right: 0; background: #1a1535; 
          border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; margin-top: 8px;
          max-height: 200px; overflow-y: auto; z-index: 100; padding: 8px;
        }
